...
void simEmp(){
	...
	int meses;
	double fin, taxa, parc, amort, finTot=0, valR, valJur;
	...
	for(int i = 1; i<=meses; i++){
		...
		parc = amort+taxa*valR;
		valR -= amort;
		finTot += parc;
		
		if(i%10==0 || i==meses || i==1)
			printf("Valor da %d prestacao: R$ %.2lf\n",i,parc);
	}
	
	valJur = finTot - fin;
	
	printf("Total pago em juros: R$ %.2lf\n", valJur);
	printf("Total do financiamento: R$ %.2lf\n",finTot);
